...
async function start(bot, chatId, username, userSessions) {
  ...
  const chat = chatId === process.env.GROUP_ADMIN || chatId === process.env.ADMIN_ID ||
    chatId === process.env.LOGIST || chatId === process.env.SERVIRCE_ID
    ...
    reply_markup: JSON.stringify({
      inline_keyboard: [
        ...
        [
          ...
          {
            text: '🔖 Обратная связь',
            web_app: { url: config.googleFormsLink },
          },
        ],
